document.addEventListener('DOMContentLoaded', function () {
    ...
    function validateTable() {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const startDate = rows[i]
                .querySelector(".startdate-field").value;

            const endDate = rows[i]
                .querySelector(".enddate-field").value;

            const hours = rows[i]
                .querySelector(".hours-field").value;
            ...
            if (new Date(endDate) < new Date(startDate)) {
                alert(
                    `Row ${i + 1}: End Date must be greater than or equal to Start Date.`
                );
                return false;
            }
            ...
            if (parseInt(hours, 10) <= 0) {
                alert(
                    `Row ${i + 1}: Number of Hours must be greater than 0.`
                );
                return false;
            }
        }
